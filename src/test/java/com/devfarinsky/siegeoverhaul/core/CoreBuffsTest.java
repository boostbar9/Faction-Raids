...
class CoreBuffsTest extends TreasuryTestSupport {
    ...
    @Test void rejectedEffectDoesNotChargeAndSuccessfulEffectChargesExactlyOnce() throws Exception {
        var player=mock(ServerPlayer.class); var treasury=fund(player,64); when(player.isAlive()).thenReturn(true);
        ...
        assertTrue(CoreBuffs.purchase(player,1)); assertEquals(64,inventory.items.get(0).getCount()); assertEquals(28,FactionBank.balance(treasury));
        when(player.hasEffect(MobEffects.DAMAGE_BOOST)).thenReturn(true);
        assertFalse(CoreBuffs.purchase(player,1)); assertEquals(64,inventory.items.get(0).getCount()); assertEquals(28,FactionBank.balance(treasury));
    }
}
