...
public final class RecruitsSiegeBridge {
    ...
    /**
     * Called once during mod bootstrap by {@code SiegeOverhaul} main class,
    ...
    private static synchronized void subscribeReflectively() {
        ...
        try {
            ...
            FactionLogger.LOG.warn(
                    "[SiegeOverhaul] Bridge Sieges could not attach: {} {} \u2014 disabled for this session.",
    ...
    private static void handleSiegeStart(Object event) {
        ...
        try {
            ...
            if (snap == null) {
                FactionLogger.LOG.debug("[SiegeOverhaul] SiegeEvent.Start received but claim snapshot came back null; ignoring.");
                return;
            }
            ...
            if (ownerTeamKey == null || ownerTeamKey.isBlank() || center == null) {
                FactionLogger.LOG.debug("[SiegeOverhaul] Skipping bridge trigger: incomplete claim snapshot (owner={}, center={}).",
                        ownerTeamKey, center);
                return;
            }
            ...
            if (triggered) {
                FactionLogger.LOG.info(
                        "[SiegeOverhaul] Bridge Sieges: SiegeEvent.Start on claim '{}' (owner team '{}') \u2192 spawned parallel Faction Raids raid at chunk {}.",
                        snap.claimName(), ownerTeamKey, center);
            } else {
                FactionLogger.LOG.debug(
                        "[SiegeOverhaul] Bridge Sieges: SiegeEvent.Start on claim '{}' declined \u2014 no matching anchor or team already at raid cap.",
                        snap.claimName());
            }
        } catch (Throwable t) {
            RUNTIME_FAILED.set(true);
            FactionLogger.LOG.warn(
                    "[SiegeOverhaul] Bridge Sieges: runtime error handling SiegeEvent.Start ({} {}) \u2014 disabling bridge for this session.",
                    t.getClass().getSimpleName(), t.getMessage());
        }
    }
    ...
    private static RecruitsClaimsBridge.ClaimSnapshot snapshotFromRaw(Object claim) {
        try {
            ...
            FactionLogger.LOG.debug("[SiegeOverhaul] Bridge Sieges: snapshotFromRaw failed: {} {}",
                    t.getClass().getSimpleName(), t.getMessage());
            return null;
        }
    }
